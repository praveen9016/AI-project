...
with open('Dataset/%s/%s_tweets.csv' % (screen_name,screen_name),encoding='utf-8',newline='') as g:
    reader = csv.reader(g)
    for row in reader:
        if i >= 1:
            entity = eval(row[3])
    ...
    with open('Dataset/s/%s_mentions_count.csv' % (screen_name,screen_name), 'wb') as f:
	     writer = csv.writer(f)
